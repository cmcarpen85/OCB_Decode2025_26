package Modules;

public class Constants {

    //FlyWheel
    public static double COASTSPEED = 0.3;
    public static double FARSHOTSPEED = 0.635;
    public static double MIDSHOTSPEED = 0.6;
    public static double CLOSESHOTSPEED = 0.4;
    public static double STARTSHOTSPEED = 0.605;

    //Hood
    public static double HOODHOME = 0.05;
    public static double HOODMINSERVOVALUE = 0.04;
    public static double HOODMAXSERVOVALUE = .6089;
    public static double HOODTARGETTOLERANCE = 1;
    public static double FARSHOTHOODSERVO = 0.608;
    public static double MIDSHOTHOODSERVO = 0;
    public static double CLOSESHOTHOODSERVO = 0;
    public static double STARTSHOTHOODSERVO = 0.6089;

    //Turret
    public static double TURRETHOME = .5;
    public static double TURRETMINANGLE = -80;
    public static double TURRETMAXANGLE = 80;
    public static double TURRETMINSERVOVALUE = 0;
    public static double TURRETMAXSERVOVALUE = 1;
    public static double TURRETANGLETOLERANCE = 1;
    public static double FARSHOTTURRETANGLE = 67;
    public static double FARSHOTTURRETSERVO = .3261;
    public static double MIDSHOTTURRETANGLE = 0;
    public static double MIDSHOTTURRETSERVO = .5;
    public static double CLOSESHOTTURRETANGLE = 0;
    public static double CLOSESHOTTURRETSERVO = .4;
    public static double STARTSHOTTURRETANGLE = 67.022;
    public static double STARTSHOTTURRETSERVO = 0.9144;
    //Slider


    //Transfer
    public static double TRANSFERPOWER = 1;
    public static double GATEPOWER = 1;
    public static double KICKERPOWER = 1;

    //Intake
    public static double INTAKEPOWER = 1;
    public static double INTAKEHOLD = 0.3;


}
